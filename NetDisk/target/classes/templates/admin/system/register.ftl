  ...
  <div class="lyear-login">
    <div class="login-center">
      ...
      <form id="register-form" method="post">
        ...
        <div class="form-group has-feedback feedback-left">
          <input type="password" placeholder="请再次输入密码" class="form-control required" id="re-password" tips="请再次填写密码" />
          <span class="mdi mdi-lock form-control-feedback" aria-hidden="true"></span>
        </div>
        <div class="form-group has-feedback feedback-left row">
          ...
          <div class="col-xs-5">
            <img src="/cpacha/generate_cpacha?vl=4&fs=25&w=128&h=40&method=admin_register" class="pull-right" id="captcha" style="cursor: pointer;" onclick="this.src=this.src+'&d='+Math.random();" title="点击刷新" alt="captcha">
          </div>
        </div>
        <div class="form-group" align="center">
          <button class="btn btn-block btn-primary" type="button" id="submit-btn">提交注册</button>
	...
	$("#submit-btn").click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.code == 0){
					showSuccessMsg('注册成功，请登录！',function(){
